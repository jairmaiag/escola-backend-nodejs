...
INSERT INTO usuario("uuid", "senha", "apelido", "email", "idpessoa", "idpapel") VALUES (uuid_generate_v4(), 'atenas399', 'O fudador', 'socrates@gmail.com',1,1);

-- Consultas 
SELECT u.uuid as uuidusu,u.apelido,u.email,p.uuid as uuidpess, p.nome,p.sobrenome,p.sexo,p.nascimento,pa.uuid as uuidpap, pa.nome AS nomepapel
	FROM usuario u 
	INNER JOIN pessoa p ON p.id=u.idpessoa
	INNER JOIN papel pa ON pa.id=u.idpapel;